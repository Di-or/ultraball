> ⚠️ **ARCHIVED — not the active spec.** This document was an alternative consolidation of the MVP design (originally draft PR #13, closing #12). The team is building against the spec produced via `/to-spec` in **[issue #14](https://github.com/Di-or/ultraball/issues/14)**, not this file. Retained for reference only; do not implement from it without reconciling against #14.
